...
namespace KaPai.Pay.My
{
    ...
    [Description("个性化替换核心模块")]
    public class MyFilterServicePack : OsharpPack
    {
        ...
        {
            // 替换原本的服务对象
            services.Replace(new ServiceDescriptor(typeof(IFilterService), typeof(MyFilterService),
                ServiceLifetime.Scoped));


            // 添加JsonDocument相关转换
            var mapperConfiguration = new MapperConfigurationExpression();
            mapperConfiguration.CreateMap<JsonDocument, string>().ConvertUsing(new JsonDocumentToString());
            mapperConfiguration.CreateMap<string, JsonDocument>().ConvertUsing(new StringToJsonDocument());


            mapperConfiguration.CreateMap<JsonDocument, IDictionary<string, object>>().ConvertUsing(new JsonDocumentToDictionary());
            mapperConfiguration.CreateMap<IDictionary<string, object>, JsonDocument>().ConvertUsing(new DictionaryToJsonDocument());

            // 替换掉默认的 AutoMapper 配置
            services.Replace(new ServiceDescriptor(typeof(MapperConfigurationExpression), mapperConfiguration));
    ...
    /// 重写FilterService
    /// </summary>
    public class MyFilterService : IFilterService
    {
        ...
        /// </summary>
        public MyFilterService(IServiceProvider serviceProvider)
        {
        ...
        {
            return MyFilterHelper.GetExpression<T>(group);
        ...
        {
            return MyFilterHelper.GetExpression<T>(rule);
        }
        ...
        {
            return MyFilterHelper.CheckFilterGroup(group, type);
        }

    }


    /// <summary>重写的查询表达式辅助操作类</summary>
    public static class MyFilterHelper
    {
        #region 字段
        ...
        private static Expression GetExpressionBody(ParameterExpression param, FilterGroup group)
        {
            ...
            throw new OsharpException("未找到对应符号");
        }
        ...
        {
            string[] propertyNames = rule.Field.Split(new[] { '.', '_' });
            Expression propertyAccess = param;
            Type type = param.Type;

            bool jsonDocumentobj = false;
            ...
            for (var index = 0; index < propertyNames.Length; index++)
            {
                ...
                //判断是不是访问的 JsonDocument对象
                if (jsonDocumentobj)
                {
                    var getInfo = propertyAccess.Type.GetMethod("GetProperty",new []{typeof(string)});

                    propertyAccess = Expression.Call(propertyAccess, getInfo ?? throw new InvalidOperationException(), Expression.Constant(propertyName, typeof(string)));

                    // 需要把最后一个参数类型 使用Call转换为属性值一样
                    if (index == propertyNames.Length - 1)
                    {
                        var valType = rule.Value.GetType();
                        propertyAccess = Expression.Call(propertyAccess, GetTypeMethodName(valType, propertyAccess));
                    }
                }
                else
                {
                    ...
                    if (property == null)
                    {
                        throw new InvalidOperationException($"在{rule.Field}中未找到{type.FullName}");
                    }
                    ...
                    // 如果访问的类型为 JsonDocument 先访问属性 RootElement  之后每个字段 都需要访问GetProperty() 方法
                    if (type == typeof(JsonDocument))
                    {
                        jsonDocumentobj = true;

                        var RootElement = type.GetProperty("RootElement");


                        propertyAccess = Expression.MakeMemberAccess(propertyAccess, RootElement);
                    }
                }
            }
        ...
        /// <returns></returns>
        private static MethodInfo GetTypeMethodName(Type valType,Expression expression)
        {
             var strname = valType.Name;
            switch (strname)
            {
                case "Int32":
                    return expression.Type.GetMethod("GetInt32");
                case "Int64":
                    return expression.Type.GetMethod("GetInt64");
                case "Int16":
                    return expression.Type.GetMethod("GetInt16");
                case "Boolean":
                    return expression.Type.GetMethod("GetBoolean");
                case "DateTime":
                    return expression.Type.GetMethod("GetDateTime");
                case "Double":
                    return expression.Type.GetMethod("GetDouble");
                case "String":
                    return expression.Type.GetMethod("GetString");
                default:
                    return expression.Type.GetMethod("GetRawText");
            }
